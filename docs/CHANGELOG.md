...
### 修复
- **性能基准文档** (2026-01-11)
  - 修正性能基准表格，区分 BM25 和 Embedding 的资源占用
  - Embedding 搜索的内存占用目标从不现实的 <100MB 修正为 ~500MB（模型内存）
  - 添加模型内存占用、首次搜索延迟等更准确的性能指标
- **PyPI 发布状态文档** (2026-01-11)
  - 统一所有文档中的 PyPI 发布状态描述
  - 移除"尚未发布到 PyPI"的过时警告
  - 更新安装指南，突出 PyPI 安装方式
- **SQLite 异常处理** (2026-01-11)
  - 在 `load_all()` 方法中添加异常日志记录
  - 避免静默跳过无效数据行时缺少诊断信息

### 改进
- **GPU 内存需求说明** (2026-01-11)
  - 在 CONFIGURATION.md 添加 GPU 内存需求表格
  - 说明不同模型的 GPU/CPU 内存占用
  - 添加模型缓存位置说明

### 文档
- 更新 `CONFIGURATION.md` 版本号至 v0.2.1
- 更新 `INSTALLATION.md` 版本号至 v0.2.1
- 更新 `ARCHITECTURE.md` 版本号至 v0.2.1
- 优化 Claude Desktop 配置示例，区分生产环境和开发环境

### 变更
- **版本号更新**: 0.2.0 → 0.2.1
- **向后兼容**: 完全兼容，仅文档更新和小幅代码改进
